...
import { postPopulateOptions, PostsService } from './posts.service';
...
export default class PostsController {
        ...
        form.parse(req, async (error: any, fields: any, files: any) => {
          ...
          if (files.photo) {
            ...
            if (metadata.width && metadata.width > 1024) {
              finalPhoto = await transformedPhoto
                .resize({ width: 1024 })
                .toBuffer();
            } else {
  ...
  async getPhoto(@Param('id') id: string, @Res() res: Response) {
    ...
    if (!photo) throw new HttpException('No photo found', HttpStatus.NOT_FOUND);
